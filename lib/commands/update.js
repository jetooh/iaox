...
import { ensureOrchestratorScaffold } from '../core/post-setup.js';
...
export async function updateCommand() {
  ...
  for (const tool of toolsWithSkill) {
    ...
    try {
      const { version, filesInstalled } = await installGodMode(projectDir, { aiTool: tool, update: true });
  ...
  // Garante os artefatos do orquestrador (memória, docs, access) que faltavam
  // em projetos antigos — idempotente, preserva dados existentes.
