...
- **Always follow @instruction.md** (project root) — the house conventions: code
  in English / app in Portuguese, per-app memory & rules folders, vertical slices,
  default tooling (knip, Playwright, 12h screenshot cleanup), and the
  create/delete-project flow.
- **Consult @.claude/memory/MEMORY.md** (the orchestrator memory index) before
  acting; open the relevant memory, and when working on an app read its
  \`.claude/memory/apps/<app>/MEMORY.md\` too.
...
export async function installGodMode(projectDir, options = {}) {
  // update=true sobrescreve arquivos de framework (skill, rules, agents,
  // instruction). Dados do usuário (ecosystem.json, config raiz, settings) são
  // sempre preservados.
  const { aiTool = 'claude-code', update = false } = options;
  ...
  // 2. Rules globais (framework) — sobrescreve no update, preserva no init
  if (toolPaths.rules) {
    ...
    if (await fse.pathExists(rulesSrc)) {
      ...
      for (const file of await fse.readdir(rulesSrc)) {
        ...
        if ((await fse.stat(src)).isFile()) {
          await fse.copy(src, path.join(rulesDest, file), { overwrite: update });
          filesInstalled++;
        }
      }
    }
  ...
  // 3.5. Documento mestre de convenções → raiz do projeto (framework)
  const instructionSrc = path.join(TEMPLATE_DIR, 'instruction.md');
  if (await fse.pathExists(instructionSrc)) {
    await fse.copy(instructionSrc, path.join(projectDir, 'instruction.md'), { overwrite: update });
    filesInstalled++;
  }

  // 3.6. Subagents customizados (@scaffolder, @security, @e2e, @i18n) → .claude/agents/
  if (resolved === 'claude-code') {
    ...
    if (await fse.pathExists(agentsSrc)) {
      ...
      await fse.ensureDir(agentsDest);
      for (const file of await fse.readdir(agentsSrc)) {
        const src = path.join(agentsSrc, file);
        if ((await fse.stat(src)).isFile()) {
          await fse.copy(src, path.join(agentsDest, file), { overwrite: update });
          filesInstalled++;
        }
      }
    }
  ...
  // 3.7. Arquivos raiz do ecossistema (ecosystem.json, package.json monorepo, turbo.json, packages/)
  const rootSrc = path.join(TEMPLATE_DIR, 'root');
  if (await fse.pathExists(rootSrc)) {
    await fse.copy(rootSrc, projectDir, { overwrite: false });
    filesInstalled += await countFiles(rootSrc);
  }
